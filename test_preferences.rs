...
use plist::{Value, Dictionary, Integer};
use dirs;
...
impl Preferences {
    fn load() -> Self {
        if let Some(prefs_path) = Self::get_preferences_path() {
            if let Ok(contents) = fs::read_to_string(&prefs_path) {
                if let Ok(prefs) = serde_json::from_str::<Preferences>(&contents) {
                    return prefs;
                }
            }
        }
    ...
    fn save_to_plist(&self) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(plist_path) = Self::get_plist_path() {
            ...
            dict.insert("refresh_interval".to_string(), Value::Integer(Integer::from(self.refresh_interval as i64)));
            dict.insert("show_notifications".to_string(), Value::Boolean(self.show_notifications));
...
impl SimplePreferencesManager {
    ...
    fn save_preferences(&self, api_url: String, api_key: String, refresh_interval: u64, show_notifications: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn show_current_preferences(&self) {
        ...
        println!("   API Key: {}...{}", &prefs.api_key[..8], &prefs.api_key[prefs.api_key.len()-4..]);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   API Key: {}...{}", &loaded_prefs.api_key[..8], &loaded_prefs.api_key[loaded_prefs.api_key.len()-4..]);
    println!("   Refresh Interval: {} seconds", loaded_prefs.refresh_interval);
    ...
    println!("   API Key: {}...{}", &manager_prefs.api_key[..8], &manager_prefs.api_key[manager_prefs.api_key.len()-4..]);
    println!("   Refresh Interval: {} seconds", manager_prefs.refresh_interval);
    println!("   Show Notifications: {}", manager_prefs.show_notifications);
    ...
    println!("   API Key: {}...{}", &reloaded_prefs.api_key[..8], &reloaded_prefs.api_key[reloaded_prefs.api_key.len()-4..]);
    println!("   Refresh Interval: {} seconds", reloaded_prefs.refresh_interval);
    println!("   Show Notifications: {}", reloaded_prefs.show_notifications);
