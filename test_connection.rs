...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api_url = "https://uptime.lazzurs.net";
    let api_key = "uk1_mmLtKoTN6mdAJJm70WSBllyuDB0cx1aSOauX0Yv8";
    ...
    if response.status().is_success() {
        ...
        if let Ok(status_pages) = serde_json::from_str::<serde_json::Value>(&response_text) {
            println!("✅ Found status pages: {}", serde_json::to_string_pretty(&status_pages).unwrap());
        }
    } else {
    ...
    let response = client
        .get(&format!("{}/api/status-page/heartbeat/your-status-page-id", api_url))
        .header("Authorization", format!("Bearer {}", api_key))
        .timeout(Duration::from_secs(10))
        .send()
        .await?;
    ...
    for (method, header_value) in auth_methods {
        ...
        if response.status().is_success() {
            ...
            if !response_text.trim().starts_with("<!DOCTYPE html>") && !response_text.trim().starts_with("<html") {
                ...
                match serde_json::from_str::<Vec<Monitor>>(&response_text) {
                    Ok(monitors) => {
                        ...
                        println!("Up: {}, Down: {}, Pending: {}", up_count, down_count, pending_count);
                        return Ok(());
                    }
                    Err(e) => {
            ...
            println!("❌ {} auth failed with status: {}", method, response.status());
        }
    }
