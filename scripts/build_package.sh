...
# Determine project name (use Cargo.toml name if available) for filenames
PROJECT_NAME=""
if [ -f Cargo.toml ]; then
...
# Sanitize PROJECT_NAME for filenames: replace spaces, forward slashes; remove surrounding quotes if any
...
log "Project name (for filenames): ${PROJECT_NAME_SAFE}"
...
if [ $DO_DMG -eq 1 ]; then
  ...
  DMG_NAME="${DIST_DIR}/${PROJECT_NAME_SAFE}-${VERSION}-${DMG_ARCH_SUFFIX}.dmg"
...
echo "  Project Name:    ${PROJECT_NAME_SAFE}"
